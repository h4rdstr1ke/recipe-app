...
export default function ProfileComplaint({ userId, onClose }: { userId: string; onClose: () => void }) {
    ...
    const handleSendComplaint = async (reasonEnum: string) => {
        ...
        try {
            ...
            await api.post(`/api/reports/users/${userId}`, payload);
            setScreen('success');
                ...
                >
                    ...
                    <div className="flex flex-col mt-[30px]">
                        <span className="px-[75px] mb-[13px] font-montserrat text-[20px] font-semibold tracking-[0.2px] leading-7">
                            Почему вы хотите пожаловаться на этот профиль?
                        </span>
                        <div>
                            <div
                                onClick={() => handleSendComplaint("CommunityRulesViolation")}
                                ...
                                <span className="font-montserrat text-[20px] font-medium tracking-[0.2px] leading-7">
                                    Автор нарушает правила сообщества
                                </span>
