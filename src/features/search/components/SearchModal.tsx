...
export default function SearchModal({ onClose }: { onClose: () => void }) {
    // Достаем из стора фильтры и функции для их изменения
    const {
        filters,
        toggleFilter,
        clearFilters,
    ...
    const handleReset = () => {
        clearFilters();
        // Модалку при сбросе не закрываем, чтобы юзер видел, что галочки снялись
    };

    return (
        <div className="fixed top-[100px] left-0 right-0 bottom-0 bg-white/80 backdrop-blur-sm z-50 flex justify-center items-start pt-[20px] overflow-y-auto pb-[40px]"
            onClick={onClose}>
            ...
            <div className="bg-[#FFFFFF] relative flex flex-col w-[780px] rounded-[15px] shadow-[0_4px_30px_rgba(0,0,0,0.08)] border border-gray-100 pb-[40px]"
                        ...
                        <div key={index} className="flex flex-col">
                            ...
                            <div className="flex flex-wrap gap-x-[15px] gap-y-[15px]">
                                {section.items.map((item, itemIndex) => {
                                    // Проверяем, выбран ли этот пункт в нашем сторе
                                    const isActive = filters[section.title]?.includes(item);

                                    return (
                                        <div
                                            key={itemIndex}
                                            onClick={() => toggleFilter(section.title, item)} // При клике добавляем/удаляем фильтр
                                            className={`border rounded-[37px] px-[20px] py-[6px] cursor-pointer transition-colors ${isActive
                                                ? 'border-[#23A6F0] bg-[#23A6F0] text-[#FFFFFF]' // Выбранный (синий фон, белый текст)
                                                : 'border-[#C4C4C4] text-[#737373] hover:border-[#23A6F0] hover:text-[#23A6F0]' // Обычный
                                                }`}
                                        >
                ...
                <div className="flex px-[70px] gap-x-[15px] items-center mt-[25px]">
                    ...
                    <button
                        type="button"
                        onClick={() => setExcludeAllergens(!excludeAllergens)}
                        className={`relative w-[45px] h-[25px] rounded-full transition-colors duration-300 ${excludeAllergens ? 'bg-[#23A6F0]' : 'bg-gray-300'}`}
                    >
                        <span
                            className={`absolute top-[2px] left-[2px] w-[21px] h-[21px] bg-white rounded-full shadow-md transition-transform duration-300 ${excludeAllergens ? 'translate-x-[20px]' : 'translate-x-0'}`}
                        />
                    </button>
                </div>

                {/* Инпуты для добавления/исключения продуктов */}
                <div className="flex flex-col px-[70px] mt-[25px] gap-5">
                    <div className="flex flex-col gap-2 relative">
                        ...
                        <input className="border border-gray-200 bg-[#F9F9F9] rounded-[8px] h-[50px] px-[20px] outline-none focus:border-[#23A6F0] font-montserrat text-[14px]" type="text" placeholder="Например: курица" />
                    </div>
                    <div className="flex flex-col gap-2 relative">
                        <h3 className="font-montserrat text-[18px] font-bold leading-7">Исключить продукт</h3>
                        <input className="border border-gray-200 bg-[#F9F9F9] rounded-[8px] h-[50px] px-[20px] outline-none focus:border-[#23A6F0] font-montserrat text-[14px]" type="text" placeholder="Например: чеснок" />
                    </div>
                </div>
                ...
                <div className="flex px-[70px] justify-end gap-[20px] mt-[40px]">
                    <button
                        type="button"
                        onClick={handleReset} // Кнопка сброса
                    ...
                    <button
                        type="submit"
                        onClick={onClose} // Закрываем модалку, фильтры уже сохранены в сторе
